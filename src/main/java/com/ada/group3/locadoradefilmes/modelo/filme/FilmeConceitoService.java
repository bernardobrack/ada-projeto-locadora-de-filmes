...
public class FilmeConceitoService {
    ...
    public List<FilmeConceitoDto> listarPorGenero(String genero) {
        return filmeConceitoRepository.findByGeneroContainingIgnoreCase(genero)
                .stream()
                .map(filmeConceito -> modelMapper.map(filmeConceito, FilmeConceitoDto.class))
                .toList();
    }

    public List<FilmeConceitoDto> listarPorTituloEGenero(String titulo, String genero) {
        return filmeConceitoRepository.findByNomeContainingIgnoreCaseAndGeneroContainingIgnoreCase(titulo, genero)
                .stream()
                .map(filmeConceito -> modelMapper.map(filmeConceito, FilmeConceitoDto.class))
                .toList();
    }
